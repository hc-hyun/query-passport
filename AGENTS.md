...
접속 준비 도구다. 현재 M1 오프라인 CLI와 M2 로컬 Docker 접속 검증을 구현했으며 발급·변경·배포 기능은 개발 중이다.
...
M1·M2의 CLI·JSON 계약은 구현과 테스트로 고정했다. M3 이후의 명령·JSON 예시는 설계안이며
현재 실행 가능한 기능으로 취급하지 않는다. 구현이 완료된 단계만 현재 상태로 갱신한다.
...
- 테스트는 root `tests/`에서 관리한다. 입력/대상 검증, 비밀정보 비노출, 계획 이후 drift,
  일부 적용 실패·복구와 실제 인증서 거부 조건을 의미 있는 테스트로 검증한다.
